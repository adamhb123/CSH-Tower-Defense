...
let LOADING_TEXT = new PIXI.Text("Loading resources...",{fontFamily : 'Verdana', fontSize: 24, fill : 0xb0197e});

function onLoadingFileProgress(e){
    // Called once per loaded/errored file
    LOADING_TEXT.text = `Loading resources... progress: ${e.progress}`;
}
function onLoadingFileError(e){
    // Called once per errored file
    let outtxt = `Error loading resources: ${e.message}`;
    LOADING_TEXT.text = outtxt;
    return outtxt;
}
...
function spriteListFromFullSheet(sheetsprite, frame_rows, frame_columns){
  ...
  for(let x = 0; x < frame_columns; x++){
    for(let y = 0; y < frame_rows; y++){
      sprites.push(new PIXI.Sprite(sheetsprite.texture, new PIXI.Rectangle(x*frame_width, y*frame_height, frame_width, frame_height)));
    }
  }
  return sprites;
}
function loadResources(resource_obj){
    ...
    return new Promise((resolve, reject)=>{
        ...
        loader.add("CSHLogoAnimationSpritesheet", "assets/misc/CSHLogoANIMATION01.png")
        .load((loader, resources) => {
          let CSHLogoAnimationSprites = spriteListFromFullSheet(new PIXI.Sprite(resources.CSHLogoAnimationSpritesheet.texture), 1, 21);
        });
