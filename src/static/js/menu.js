...
function buy_menu(stage){
  ...
  show_money.addBMText(MONEY.money, 64, 0.5, 0.5);
  ...
  let defender_buy_cards = DEFENDERS.map( (defender, index) => {
    ...
    let buy_card = new Entities.ClickableEntity(defender.name, defender_menu, BUY_CARD_GAP_SIZE.x * (index % 3) + BUY_CARD_POSITION.x, BUY_CARD_GAP_SIZE.y * Math.floor(index % 6 / 3) + BUY_CARD_POSITION.y, defender.animations, null, true);
    buy_card.scale(1.5);
  ...
  function renderPageForDefender(defender){
    ...
    let anim = new Entities.Entity("anim", background, Utility.scaleX(0.09), Utility.scaleY(0.1), defender.animations);
